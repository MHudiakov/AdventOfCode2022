...
const int workTime = 24;
var blueprints = ReadInput();

int qualityLevelSum = (from blueprint in blueprints
    let maxGeodeNumber = DetermineMaxGeodeNumber(blueprint)
    select blueprint.Id * maxGeodeNumber).Sum();

Console.WriteLine(qualityLevelSum);
